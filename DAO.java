...
public class DAO {
	...
	public boolean inserirUsuario(Usuario usuario) {
		...
		try {  
			...
			st.executeUpdate("INSERT INTO usuario (codigo, login, senha, CPF) "
	...
	public boolean atualizarUsuario(Usuario usuario) {
		...
		try {  
			...
			String sql = "UPDATE usuario SET login = '" + usuario.getLogin() + "', senha = '"  
				       + usuario.getSenha() + "', CPF = '" + usuario.getCPF() + "'"
	         ...
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	                usuarios[i] = new Usuario(rs.getInt("codigo"), rs.getString("login"), 
	                		                  rs.getString("senha"), rs.getString("CPF").charAt(0));
	             }
	          }
	          st.close();
		} catch (Exception e) {
	...
	public Usuario[] getUsuariosMasculinos() {
		...
		try {
			Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
			ResultSet rs = st.executeQuery("SELECT * FROM usuario WHERE usuario.CPF LIKE 'M'");		
	         if(rs.next()){
	             rs.last();
	             usuarios = new Usuario[rs.getRow()];
	             rs.beforeFirst();
